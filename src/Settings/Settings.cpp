// Copyright (c) 2015-2017, The Hexagon developers
//
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hexagonwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hexagonwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hexagonwallet.desktop");
  ...
  res = autorunSettings.contains("HexagonWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("HexagonWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.hexagon.hexagonwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Hexagon Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("HexagonWallet", appPath);
    } else {
      autorunSettings.remove("HexagonWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hexagon", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:hexagon");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hexagon\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hexagon\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hexagonwallet.cfg"));
