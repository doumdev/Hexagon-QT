...
private:
  ...
  const QDir m_hexagonDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfHexagonBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _hexagon) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo hexagonBlockchainInfo() const;
