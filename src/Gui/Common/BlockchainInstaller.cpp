...
const quint32 HEXAGON_BLOCK_SIZE = 0xd5;
...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_hexagonDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfHexagonBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 hexagonHeight;
  if (!checkIfBlockchainOutdated(currentHeight, hexagonHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(hexagonHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::getGenesisBlockFromBlockchain(char** _genesisBlockData) const {
  ...
  if (blockSize != HEXAGON_BLOCK_SIZE) {
    return false;
  }
...
bool BlockchainInstaller::checkIfHexagonBlockchainExists() const {
  return m_hexagonDir.exists() && m_hexagonDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_hexagonDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::hexagonBlockchainInfo() const {
  return QFileInfo(m_hexagonDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_hexagonDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_hexagonDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
