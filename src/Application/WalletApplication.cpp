...
const char HEXAGON_URI_SCHEME_NAME[] = "hexagon";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("hexagonwallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("hexagonwallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != HEXAGON_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Hexagon wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Hexagon wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have hexagond with non-default RPC port, you should terminate it and relaunch Hexagon Wallet\n"
      "or\n"
      "Set the Local deamon required port in Hexagon Wallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
